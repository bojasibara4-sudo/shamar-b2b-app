  ...
  const { data: order, error } = await (supabase as any)
    ...
    .select(`
      *,
      ...
      buyer:users!orders_buyer_id_fkey(id, email, full_name, company_name, phone),
      payments:payments(
        id,
        status,
        amount_total,
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            <div className="bg-slate-50 rounded-[1.5rem] border border-slate-100 p-6">
              <div className="flex items-center gap-3 mb-3">
                ...
                <h3 className="font-black text-slate-900">Acheteur</h3>
              </div>
              <p className="text-slate-900 font-bold text-lg">{order.buyer?.company_name || order.buyer?.full_name || 'N/A'}</p>
              <p className="text-sm text-slate-600 font-medium">{order.buyer?.email}</p>
              {order.buyer?.phone && (
                <p className="text-sm text-slate-600 font-medium">{order.buyer.phone}</p>
              )}
            </div>
