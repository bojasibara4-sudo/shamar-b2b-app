...
export default async function ProfileInfoPage() {
  ...
  const { data: profile } = await (supabase as any).from('users').select('*').eq('id', user.id).single();
  ...
  return (
    <div className="max-w-shamar-container mx-auto px-4 py-shamar-24">
      ...
      <h1 className="text-shamar-h2 text-gray-900">Infos personnelles</h1>
      <p className="text-shamar-body text-gray-500 mt-1">Nom, email, téléphone, entreprise, adresse, pays, ville.</p>
