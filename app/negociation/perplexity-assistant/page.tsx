      ...
      <div className="space-y-8 animate-in fade-in duration-500">
          ...
          <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-4">
            <div>
              <h1 className="text-4xl font-black text-slate-900 tracking-tighter mb-2 flex items-center gap-3">
                ...
                Assistant Négociation <span className="text-emerald-600">Perplexity AI</span>
              </h1>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
          ...
          <div className="bg-white rounded-[2rem] border border-slate-200 shadow-sm p-8">
            <h2 className="text-2xl font-black text-slate-900 mb-6">Paramètres</h2>

          {activeTab === 'arguments' && (
            <div className="space-y-4">
              <div>
                <label className="block text-sm font-black text-slate-700 mb-2">Produit</label>
                <input
                  type="text"
                  value={negotiationContext.commodity}
                  onChange={(e) => setNegotiationContext({ ...negotiationContext, commodity: e.target.value })}
                  className="w-full px-4 py-3 border-2 border-slate-200 rounded-xl focus:ring-2 focus:ring-emerald-500 focus:border-emerald-500 outline-none transition-all font-medium"
                  placeholder="Ex: Cacao, Café, Huile de palme..."
                />
              </div>
              <div className="grid grid-cols-2 gap-4">
                <div>
                  <label className="block text-sm font-black text-slate-700 mb-2">Quantité</label>
                  <input
                    type="number"
                ...
                <div>
                  <label className="block text-sm font-black text-slate-700 mb-2">Unité</label>
                  <select
              ...
              <div className="grid grid-cols-2 gap-4">
                <div>
                  <label className="block text-sm font-black text-slate-700 mb-2">Prix proposé</label>
                  <input
                    type="number"
                ...
                <div>
                  <label className="block text-sm font-black text-slate-700 mb-2">Devise</label>
                  <select
              ...
              <button
                ...
                className="w-full px-6 py-4 bg-emerald-600 text-white rounded-xl font-black hover:bg-emerald-700 transition-all disabled:opacity-50 disabled:cursor-not-allowed shadow-lg shadow-emerald-600/20"
              >
          ...
          {activeTab === 'prices' && (
            <div className="space-y-6">
              ...
              <div className="grid grid-cols-2 gap-4">
                <div>
                  <label className="block text-sm font-black text-slate-700 mb-2">Prix Chine</label>
                  <input
                    type="number"
                ...
                <div>
                  <label className="block text-sm font-black text-slate-700 mb-2">Prix marché local</label>
                  <input
                    type="number"
          ...
          {activeTab === 'objections' && (
            <div className="space-y-6">
              <div>
                <label className="block text-sm font-black text-slate-700 mb-2">Objection client</label>
                <textarea
                  value={objection}
              ...
              <div>
                <label className="block text-sm font-black text-slate-700 mb-2">Contexte</label>
                <textarea
          ...
          {activeTab === 'messages' && (
            <div className="space-y-6">
              <div>
                <label className="block text-sm font-black text-slate-700 mb-2">Destinataire</label>
                <select
          ...
          {/* Response */}
          <div className="bg-white rounded-[2rem] border border-slate-200 shadow-sm p-8">
            <h2 className="text-2xl font-black text-slate-900 mb-6">Réponse Perplexity AI</h2>
            ...
            {error && (
              <div className="p-6 bg-red-50 border-2 border-red-200 rounded-xl text-red-700">
                <p className="font-black text-lg mb-2">Erreur</p>
            ...
            {response && !loading && (
              <div className="p-6 bg-emerald-50 border-2 border-emerald-200 rounded-xl">
                <div className="prose max-w-none">
                  <p className="text-slate-800 whitespace-pre-wrap font-medium leading-relaxed">{response}</p>
                </div>
              </div>
            )}
            ...
            {!response && !loading && !error && (
              <div className="py-16 text-center text-slate-400">
                ...
                <p className="font-medium">Remplissez le formulaire et générez une réponse</p>
              </div>
            )}
          </div>
        </div>
