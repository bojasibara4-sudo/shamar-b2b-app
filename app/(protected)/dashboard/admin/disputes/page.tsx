import { requireAdmin } from '@/lib/auth-guard';
import { getAdminDisputes } from '@/services/dispute.service';
import Link from 'next/link';
import { ShieldAlert } from 'lucide-react';

export const dynamic = 'force-dynamic';

export default async function AdminDisputesPage() {
  await requireAdmin();

  const disputes = await getAdminDisputes();

  return (
    <div className="bg-gray-50 min-h-full">
      <div className="max-w-shamar-container mx-auto px-4 sm:px-6 lg:px-8 py-shamar-24">
      <div className="space-y-shamar-32 animate-in fade-in duration-500">
        <div className="bg-gray-0 rounded-shamar-md border border-gray-200 p-shamar-32 shadow-shamar-soft">
          <div className="flex items-center gap-shamar-16 mb-2">
            <div className="p-3 bg-red-100 rounded-shamar-md">
              <ShieldAlert className="text-red-600" size={32} />
            </div>
            <div>
              <h1 className="text-shamar-h1 text-gray-900 tracking-tight">
                <span className="text-primary-600">Litiges</span>
              </h1>
              <p className="text-shamar-body text-gray-500 font-medium mt-1">
                Gestion des litiges et réclamations
              </p>
            </div>
          </div>
        </div>

        <div className="bg-gray-0 rounded-shamar-md border border-gray-200 shadow-shamar-soft p-shamar-32">
          <h2 className="text-shamar-h3 font-semibold text-gray-900 mb-shamar-16">Liste des litiges</h2>
          {disputes.length === 0 ? (
            <div className="text-center py-shamar-48">
              <div className="w-20 h-20 bg-gray-100 rounded-full flex items-center justify-center mx-auto mb-shamar-24">
                <ShieldAlert className="h-10 w-10 text-gray-400" />
              </div>
              <p className="text-gray-500 font-medium text-shamar-body">Aucun litige en cours.</p>
              <p className="text-gray-500 text-shamar-small mt-2">Tout est calme pour le moment.</p>
            </div>
          ) : (
            <div className="space-y-0 divide-y divide-gray-200">
              {disputes.map((d: any) => (
                <Link
                  key={d.id}
                  href={`/dashboard/admin/disputes/${d.id}`}
                  className="block py-shamar-16 px-shamar-16 rounded-shamar-md border border-transparent hover:border-gray-200 hover:bg-gray-50 transition-colors first:pt-0"
                >
                  <div className="flex justify-between items-center">
                    <span className="font-medium text-gray-900 text-shamar-body">{d.reason} — #{d.order_id?.slice(0, 8)}</span>
                    <span className={`px-3 py-1 rounded-shamar-sm text-shamar-small font-semibold ${
                      d.status === 'resolved' ? 'bg-success-500/20 text-emerald-700' :
                      d.status === 'rejected' ? 'bg-red-100 text-red-700' :
                      'bg-warning-500/20 text-amber-700'
                    }`}>
                      {d.status === 'open' ? 'Ouvert' : d.status === 'resolved' ? 'Résolu' : 'Rejeté'}
                    </span>
                  </div>
                  {d.description && <p className="text-shamar-small text-gray-500 mt-1 line-clamp-1">{d.description}</p>}
                </Link>
              ))}
            </div>
          )}
        </div>
      </div>
    </div>
    </div>
  );
}
