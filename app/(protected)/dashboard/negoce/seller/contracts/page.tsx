...
import { getNegoceContractsBySupplier } from '@/services/negoce.service';

export const dynamic = 'force-dynamic';

export default async function NegoceSellerContractsPage() {
    ...
    <div className="bg-gray-50 min-h-full">
      <div className="max-w-shamar-container mx-auto px-4 sm:px-6 lg:px-8 py-shamar-24">
        <div className="space-y-shamar-32 animate-in fade-in duration-500">
          <h1 className="text-shamar-h1 text-gray-900 tracking-tight">Contrats actifs</h1>
              ...
              <div className="space-y-shamar-16">
                {contracts.map((c: any) => (
                  <Link key={c.id} href={`/negoce/contract/${c.offer_id}`} className="block p-shamar-16 rounded-shamar-md bg-gray-50 hover:bg-gray-100 transition-colors">
                    ...
                    <p className="text-shamar-small text-gray-500">{c.total_amount} {c.currency}</p>
                  </Link>
                ))}
              </div>
            ) : (
